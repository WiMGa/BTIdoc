...
- `CMongoDB.cs` - подключение к MongoDB для координации
- `CTask.cs` - структура задания для обработки
- Поддерживающие классы: `CParams.cs`, `CVector.cs`

**Функции**:
- Загрузка векторов из `c:\mega\DBM\vectors_data.json`
- Создание заданий для обработки каждого вектора
- Координация выполнения через MongoDB
...
**Функции**:
- Получение заданий из очереди MongoDB
- Выполнение k-NN оптимизации для конкретного вектора
- Сохранение результатов обратно в MongoDB
...
## Координация через MongoDB

### База данных: bti_db
**Коллекция tasks**:
- Хранит все задания для обработки
- Статусы: "new" → "processing" → "completed"
- Атомарные операции для предотвращения конфликтов

### Рабочий процесс
1. **BTIman** создаёт задания со статусом "new"
2. **BTIwork** экземпляры забирают задания (статус → "processing")
3. После обработки статус становится "completed" с результатами
